...
def investment_types(Investment_Category, Investment_Type, Investment_Name, Payout_Rate, Cost, Time_Sold):
    database = r"/mnt/sda1/database_test/dam_bot.db"
    conn = sqlite3.connect(database)
    with conn:
        sql = f'''INSERT INTO investment_details(Investment_Category, Investment_Type, Investment_Name, Payout_Rate, Cost, Time_Sold)
            VALUES(?,?,?,?,?,?)'''
        cur = conn.cursor()
        project = (Investment_Category, Investment_Type, Investment_Name, Payout_Rate, Cost, Time_Sold)
        cur.execute(sql, project)
